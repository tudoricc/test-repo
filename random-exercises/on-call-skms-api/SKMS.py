...
Copyright (c) 2013-2017 Adobe Systems Inc (TechOps AppDev)
...
class WebApiClient(object):
    ...
    skms_domain = 'api.skms.adobe.com'
    ...
    # ---------------------------#
    def set_custom_client_type(self, client_type_string=CLIENT_TYPE):
        """Enables a custom setting for the client type"""
        self.CLIENT_TYPE = client_type_string

    def get_client_type(self):
        """Return the current client type value"""
        return self.CLIENT_TYPE

    def set_custom_client_version(self, client_version_string=CLIENT_VERSION):
        """Enables a custom setting for the client version"""
        self.CLIENT_VERSION = client_version_string

    def get_client_version(self):
        """Return the current client version value"""
        ...
        if os.path.isfile(skms_session_storage_file):
            ...
            if file_contents != '':
                ...
                if (
                    ...
                    session_info['skms_session_id'] is not None and
                ...
                if (
                    ...
                    session_info['skms_csrf_token'] is not None and
    ...
    def send_request(self, object_name, method_name, method_param_dict=None, rate_limit_retries=25):
        ...
        try:
            ...
            if status.lower() == "success":
                ...
                if error_message_list.__len__() > 1:
                    ...
                    if error_message_list[0]['error_code'] == 'API_USER_RATE_LIMIT_REACHED' and rate_limit_retries > 0:
                        time.sleep(error_message_list[0]['error_data']['retry_seconds'])
                        return self.send_request(object_name, method_name, method_param_dict, rate_limit_retries - 1)
