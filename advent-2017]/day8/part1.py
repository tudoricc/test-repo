...
with open('part1.txt') as f:
    ...
    for i in range(0,len(variables)):
        # print "Comparison returns " + str(ops[operator[i]]( int(values_dict[variable_for_comparison[i]]) , int(denominator[i]) ) ) + " for " + str(variable_for_comparison[i]) + " " + str(operator[i]) + " and " + str(denominator[i])
        # print "Before modifications  " + str(values_dict)
        #
        # print "----------------------------"
        if ops[operator[i]](  int(values_dict[variable_for_comparison[i]]) ,  int(denominator[i]) ) == True:
            ...
            #print "Result is " + str(value_to_be_added)
            values_dict[ variables[i] ] = value_to_be_added
            #print ops[function[i]]
            #print "After  modifications  " + str(values_dict)
